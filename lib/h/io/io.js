...
h.io.__createMessage = function(incomingMessage, payload) {
  console.log('LOG: ', incomingMessage.trailers);
  var messageUrl = String(incomingMessage.url || '');
  var decodedUrl = codec.url.decode(messageUrl);
  ...
  message.setPayload(h.codec.decodePayload(
      payload,
      String(decodedUrl['search'] || '').slice(1),
